function [acc_ft, acc_svm_ft, acc_svm_pt] = train_svm(nets, data)
...
disp('training finetuned')
[svm.fine_tuned.predictions, svm.fine_tuned.accuracy] = get_predictions(svm.fine_tuned);

acc_ft = nn.accuracy * 100;
acc_svm_ft = svm.fine_tuned.accuracy(1);
acc_svm_pt = svm.pre_trained.accuracy(1);
...
disp('Training')
model = train(data.trainset.labels, data.trainset.features, sprintf('-c %f -s 0', C)); % use the same solver: -s 0

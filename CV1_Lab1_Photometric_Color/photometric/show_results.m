...
mesh(X, Y, squeeze(SE), gradient(squeeze(SE)));
...
% normal_img = squeeze(normals); %squeeze(mean(normals, 3)); %zeros(size(normals));
% minN = min(min(normal_img, [], 1), [], 2);
% maxN = max(max(normal_img, [], 1), [], 2);
% normal_img = (normal_img - minN) ./ (maxN - minN);

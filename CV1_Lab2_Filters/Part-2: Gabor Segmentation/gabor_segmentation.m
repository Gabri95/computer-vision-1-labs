...
for jj = 1 : length(gaborFilterBank)
    ...
    real_out =  imfilter(im2double(img_gray), filter(:, :, 1));% \\TODO: filter the grayscale input with real part of the Gabor
    imag_out =  imfilter(im2double(img_gray), filter(:, :, 2));% \\TODO: filter the grayscale input with imaginary part of the Gabor
    featureMaps{jj} = cat(3, real_out, imag_out);
    
    % Visualize the filter responses if you wish.
    if visFlag
        figure(2),
        subplot(121), imshow(real_out), title(sprintf('Re[h(x,y)], \\lambda = %f, \\theta = %f, \\sigma = %f, max = %f',gaborFilterBank(jj).lambda,...
                                                                                                              gaborFilterBank(jj).theta,...
                                                                                                              gaborFilterBank(jj).sigma,...
                                                                                                              max(real_out(:))));
        subplot(122), imshow(imag_out), title(sprintf('Im[h(x,y)], \\lambda = %f, \\theta = %f, \\sigma = %f, max = %f',gaborFilterBank(jj).lambda,...
                                                                                                              gaborFilterBank(jj).theta,...
                                                                                                              gaborFilterBank(jj).sigma,...
                                                                                                              max(imag_out(:))));
        pause(1)
...
for jj = 1:length(featureMaps)
    ...
    if visFlag
        figure(3), 
        fm = (featureMags{jj});
        imshow((featureMags{jj})), title(sprintf('Re[h(x,y)], \\lambda = %f, \\theta = %f, \\sigma = %f, max = %f',gaborFilterBank(jj).lambda,...
                                                                                                              gaborFilterBank(jj).theta,...
                                                                                                              gaborFilterBank(jj).sigma,...
                                                                                                              max(fm(:))));
        pause(.3)    
...
if smoothingFlag
    ...
    for jj = 1:length(featureMags)
        smoothed = imgaussfilt(featureMags{jj}, sigmaSmoothing);
